...
{	if (refined())
	...
	else {	DEBUG(std::cout << this << ": " << str() << std::endl;)
		points.sort(sort_root_at_label);
		DEBUG(std::cout << "  Points sorted." << std::endl;)
		for (Waypoint *w : points)
		  if (w->colocated && w == w->colocated->front()) w->colocated->sort(sort_root_at_label);
		DEBUG(std::cout << "  Colocation lists sorted." << std::endl;)
	     }
}
